...
The numbers generated by `getFakeMathRandom` and `SeededRng` are no longer consistent with previous seeds.

### Fixes

Fixed the that number generation using `getFakeMathRandom` produced inconsistent results across Deno and Node.js runtimes.

### Internal features

Added `IntegerSeed` class:
It generates deterministic sequences of integer seeds in the range `[0, 2^31-1]` from any numeric input and is responsible for all seed generation in the library.

### Refactoring

Refactored `getFakeMathRandom` to use an algorithm for generation of pseudo-random numbers that is more consistent across runtimes. It is the basis of all pseudo-random number generation in the library.

Refactored the `SeededRng` class:

- The seed is now an integer in every case, and it always has a maximum range `[0, 2^31-1]`.
- `IntSeededRng` and `Int32SeededRng` now have a seed range identical to that of `SeededRng`.
  The difference is that their `next()` methods return integers instead of fractional numbers.
